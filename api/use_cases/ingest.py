...
class IngestUseCase:
    ...
    def _create_pdf_parser(self):
        """Create PDF parser based on configuration.

        Returns:
            PDF parser instance (PyMuPdfParser or PdfParser)
        """
        if self.config.pdf_parser == "pymupdf":
            # Use PyMuPDF with optional Gemini Vision OCR
            ocr = None
